...
class StackFrontier:
    ...
    def contains_state(self,state : Node.state):
...
class BestFirstFrontier(StackFrontier):
    ...
    def add(self,node):
        self.frontier.append((node, self.calculate_manhattan(node)))
        self.frontier.sort(key = lambda x:x[1], reverse = False)
    ...
    def calculate_manhattan(self,node):
        x,y = node.state
        return abs(x-self.a) + abs(y-self.b)

        
class AStarSearch(BestFirstFrontier):
...
if __name__ == '__main__':
    ...
    print(a.solution)
    a.print()
    a.output_image("maze.png", show_explored=True)
